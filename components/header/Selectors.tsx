  ...
  return (
    <div className="relative" ref={ref}>
      <button 
        ...
        className="h-8 sm:h-10 px-2 sm:px-3 flex items-center gap-1.5 sm:gap-2 border border-brand-accent/30 bg-brand-accent/5 text-[9px] sm:text-[10px] font-bold tracking-widest hover:bg-brand-accent/10 transition-all text-brand-accent rounded-sm shrink-0"
      >
        ...
        <ChevronDown className={`hidden sm:inline w-2.5 h-2.5 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
...
export const AttemptSelector: React.FC<{
  ...
  return (
    <div className="relative" ref={ref}>
       <button 
         onClick={() => setIsOpen(!isOpen)}
         className="h-8 sm:h-10 px-2 sm:px-3 flex items-center gap-1.5 sm:gap-2 border border-brand-cyan/30 bg-brand-cyan/5 text-[9px] sm:text-[10px] font-bold tracking-widest hover:bg-brand-cyan/10 transition-all text-brand-cyan rounded-sm shrink-0"
       >
         <History className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
         <span className="max-w-[70px] sm:max-w-none truncate">{currentLabel}</span>
         <ChevronDown className={`hidden sm:inline w-2.5 h-2.5 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
       </button>
