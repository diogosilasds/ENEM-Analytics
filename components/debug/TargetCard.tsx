        ...
        <div className={`relative ${cardBg} border ${borderClass} p-4 sm:p-5 flex flex-col justify-between group hover:brightness-110 transition-all duration-300 h-full overflow-hidden`}>
            ...
            <div className="relative z-10 flex justify-between items-start mb-4 sm:mb-6">
                ...
                <div className="flex flex-col items-end text-brand-accent shrink-0 ml-2">
                    <span className="text-[9px] font-mono opacity-70">NOTA</span>
                    <span className="text-xl sm:text-2xl font-black font-display leading-none">{target.priorityLevel}</span>
                </div>
            </div>
            <div className="relative z-10 space-y-4">
                <div className="space-y-1">
                    <div className="flex justify-between text-[9px] font-mono text-brand-muted uppercase"><span>Fator de Impacto (Gap)</span><span className="text-white font-bold">{target.impactScore.toFixed(0)}</span></div>
                    <div className="h-1.5 w-full bg-black/50 border border-white/10">
                        <div className={`h-full ${gaugeColor} transition-all duration-1000`} style={{ width: `${Math.min((Math.max(0, target.impactScore) / 750) * 100, 100)}%` }}></div>
                    </div>
                </div>
                <div className="space-y-1">
                     <div className="flex justify-between text-[9px] font-mono text-brand-muted uppercase"><span>Precisão (Até 750)</span><span className="text-white font-bold">{target.accuracy.toFixed(0)}%</span></div>
